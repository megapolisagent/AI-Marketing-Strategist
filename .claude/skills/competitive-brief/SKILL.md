...
>
> **Пробел переноса**: ссылка ниже на `CONNECTORS.md` ведёт на файл из оригинального плагина Anthropic (`marketing/CONNECTORS.md`, описывает подключение MCP вроде HubSpot/Similarweb/Ahrefs) — этот файл не перенесён, у AI Marketing Strategist таких коннекторов сейчас нет (только Exa/Firecrawl). Ссылка оставлена как есть, не выдумана замена; при отсутствии этих MCP скилл работает через обычный WebSearch/WebFetch/Exa/Firecrawl, просто без части «Research Sources» (SEO-инструменты, финансовые данные) в полном объёме.
