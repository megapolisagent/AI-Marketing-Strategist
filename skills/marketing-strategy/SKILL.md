...
**Реальный механизм «чата вызвавшей сессии» — не метафора.** Если сессия соседнего агента (Авитолог, ENGINEER, MAIN_ASSISTANT) сейчас открыта на этой машине — передавай итог функцией `SendMessage` (Claude Code, доставляет сообщение в открытую сессию по имени; `ListAgents` показывает, кто сейчас открыт). Это тот же native-инструмент, что уже использует MAIN_ASSISTANT для делегирования (`MAIN_ASSISTANT/HOME.md`, «Делегирование») — не MCP и не отдельный API, штатная функция среды. Если нужного агента среди открытых сессий нет — скажи владельцу прямо, какую сессию открыть, не делай вид, что итог передан. Асинхронный путь (сессия не открыта прямо сейчас) — уже описан выше: `knowledge/businesses/<слаг>.md`, файл читает адресат в следующую свою сессию. Исправлено по итогам независимого аудита Codex (2026-09-02, `ENGINEER/audit_marketer_report.md`) — раньше связь была продекларирована («отдать итог в чат»), но без названного механизма.
